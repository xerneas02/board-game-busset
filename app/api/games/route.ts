...
export async function GET() {
  return NextResponse.json(db.prepare("SELECT g.*, (SELECT count(*) FROM plays p WHERE p.gameId=g.id) playedCount, (SELECT ROUND(AVG(p.duration)) FROM plays p WHERE p.gameId=g.id AND p.duration IS NOT NULL) avgDuration FROM games g WHERE active=1 ORDER BY name").all());
}
...
export async function POST(request:NextRequest) {
  ...
  try {
    const result=db.prepare("INSERT INTO games(slug,name,coverPath,minPlayers,maxPlayers,estimatedMinutes,difficulty,rating,notes,rulesUrl) VALUES(?,?,?,?,?,?,?,?,?,?)").run(slug,name,body.coverPath||null,body.minPlayers,body.maxPlayers,body.estimatedMinutes||null,body.difficulty||null,body.rating||null,body.notes||null,body.rulesUrl||null);
