...
export async function GET(request:NextRequest) {
  ...
  const terms=q.normalize("NFD").replace(/[\u0300-\u036f]/g,"").match(/[a-zA-Z0-9]+/g);
  if (!terms) return NextResponse.json({sections:[],documents:[],results:[]});
  const query=terms.map(term=>`"${term}"*`).join(" AND ");
  const sections=db.prepare("SELECT s.id,s.title,s.content FROM rules_fts f JOIN rule_sections s ON s.id=f.rowid WHERE s.gameId=? AND rules_fts MATCH ? LIMIT 20").all(id,query);
  const pages=db.prepare("SELECT p.id,p.page,p.content,d.filePath,d.fileName FROM rule_pages_fts f JOIN rule_pages p ON p.id=f.rowid JOIN rule_documents d ON d.id=p.documentId WHERE p.gameId=? AND rule_pages_fts MATCH ? LIMIT 20").all(id,query);
