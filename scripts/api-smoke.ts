...
async function call<T=any>(path:string,method="GET",body?:unknown):Promise<T>{const response=await fetch(base+path,{method,headers:body?{"Content-Type":"application/json"}:undefined,body:body?JSON.stringify(body):undefined});if(!response.ok)throw new Error(`${method} ${path}: ${response.status} ${await response.text()}`);return response.status===204?undefined as T:await response.json() as T}
...
async function main(){
  ...
  const game=await call("/api/games","POST",{name:"Essai "+suffix,minPlayers:2,maxPlayers:4,estimatedMinutes:45,difficulty:1.5});
  const duplicate=await fetch(base+"/api/games",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:"  ESSAI "+suffix+"  ",minPlayers:2,maxPlayers:4})});
  ...
  const uploadResponse=await fetch(base+"/api/media",{method:"POST",body:form});assert.equal(uploadResponse.status,200);
